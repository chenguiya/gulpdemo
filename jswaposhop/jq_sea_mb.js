...
var wapmodule={
	...
	PrivateMessage:function(userid,username){
		var name=encodeURIComponent(username);
		if(isIOS){
			window.location.href='ios://NativePrivateMessage/'+userid+'/'+name;
		}else if(isAndroid){
			try{
				window.Android.NativePrivateMessage(userid,username)
			}catch(e){
				showmsg('此版本不支持，请安装最新版本','',2000);
			}			
		}else{
      showmsg('请安装app进行客服沟通','',2000);
		}
	}
};
...
seajs.config({
	...
	alias: {
		"$": "sea_js/jquery.2.1.1.min.js",
		"jquery": "sea_js/jquery.2.1.1.min.js",
		'seajs-css': 'js/seajs-css/1.0.4/seajs-css.js',
		"swiper": "sea_js/swiper"
	},
	charset: 'utf-8',
	debug: true
});
