define("test/mobile/recruit_app",["jquery","common","mobtime","js/module/mobtime/mobtime.css","http://res.wx.qq.com/open/js/jweixin-1.0.0.js"],function(a){
    ...
    $.ajax({
        ...
        success:function(e){
           ...
           wx.config({
              ...
              jsApiList:["checkJsApi","onMenuShareTimeline","onMenuShareAppMessage","getLocation","openLocation"]
            })
        },
        error:function(){
           showmsg('数据有误','',1000)
   ...
   /* $('#getLocation').click(function(){
       var latitude='23.127977';//宝生-获取的纬度
        var longitude='113.248535';//宝生-获取的经度
            var Arr=[longitude,latitude];
            var script=document.createElement('script');
            var paramUrl='http://api.map.baidu.com/geoconv/v1/?coords='+Arr.toString()+'&from=1&to=5&ak=820fef90c4bf9795c46a5111e7b4f9d2&callback=dealResult';
            script.src=paramUrl;
...
function renderReverse(msg){
   ...
   $.ajax({
      ...
      data:{tid:recruitTid,latitude:msg.result.location.lat,longitude:msg.result.location.lng,province:msg.result.addressComponent.province,city:msg.result.addressComponent.city,district:msg.result.addressComponent.district,address:msg.result.formatted_address},
      dataType:'json',
      cache:false,
      success:function(data){
         if(data.code==200){
